...
const suggestedCompanies = [
  {
    ...
    industry: 'Technology',
    interestMatch: 'High',
    feedback: 'Highly rated by previous interns for mentorship and growth.',
  },
  {
    id: 's2',
    name: 'HealthSys Analytics',
    industry: 'Healthcare',
    interestMatch: 'Moderate',
    feedback: 'Great for data science and research internships.',
  }
];
...
export default function InternshipListing() {
  ...
  const [viewMode, setViewMode] = useState('internships');
  ...
  return (
    <div className={styles.pageLayout}>
      ...
      <div className={styles.mainContent}>
        <Toolbar title="Internships" />

        <div className={styles.viewToggle}>
          <button
            className={`${styles.toggleBtn} ${viewMode === 'internships' ? styles.active : ''}`}
            onClick={() => setViewMode('internships')}
          >
            Available Internships
          </button>
          <button
            className={`${styles.toggleBtn} ${viewMode === 'suggested' ? styles.active : ''}`}
        ...
        {viewMode === 'internships' && (
          <>
            <h2 className={styles.availableTitle}>Available Internships</h2>
                ...
                filteredInternships.map((internship) => (
                  <div key={internship.id} className={styles.internshipCard}>
                    <div className={styles.cardHeaderRow}>
                      ...
                      <Image
                        src={brokenImages[internship.id] ? '/images/default-logo.png' : internship.image}
        ...
        {viewMode === 'suggested' && (
          <div className={styles.internshipList}>
            <h2 className={styles.suggestedTitle}>Suggested Companies</h2>
            {suggestedCompanies.map((company) => (
              <div key={company.id} className={styles.internshipCard}>
                <h3>{company.name}</h3>
                <p><strong>Industry:</strong> {company.industry}</p>
                <p><strong>Match:</strong> {company.interestMatch}</p>
                <p><strong>Feedback:</strong> {company.feedback}</p>
              </div>
            ))}
          </div>
        )}
